...
def parse_parameters(prompt: str) -> dict:
    ...
    if not len_match: raise ValueError("Missing Essential Parameter: 'length' required for fastener extraction.")
    ...
    dia_val = float(dia_match.group(1)) if dia_match else BOLT_DEFAULTS["diameter"]
